...
public class ActionsListWindow extends BOWindow
{
    ...
    /**
     * Constructs a new action list window attached to the given container screen.
     *
     * @param attachedToScreen the container screen this window is attached to.
     * @param layout           the widget layout configuration for positioning.
     */
    public ActionsListWindow(final AbstractContainerScreen<?> attachedToScreen, final WidgetLayout layout)
    {
        ...
        final Button toggleBtn = findPaneOfTypeByID("favorites_toggle", Button.class);
        ...
        for (int pageId = 0; pageId < pageCount; pageId++)
        {
            ...
            final SnappingScrollingList list = (SnappingScrollingList) Loader.createFromXMLFile(modId("gui/widgetrow.xml"), pageRoot);
            ...
            list.setDataProvider(new DataProvider()
            {
                ...
                public void updateElement(final int index, final Pane rowPane)
                {
                    for (int groupOffset = 0; groupOffset < layout.getMaxGroups(); groupOffset++)
                    {
                        ...
                        if (editMode)
                        {
                            button.setHandler(btn -> onEditModeWidgetClick(widget));
                        }
                        else
                        {
            ...
            public void updateElement(final int index, final Pane rowPane)
            {
                ...
                for (int col = 0; col < favCols; col++)
                {
                    ...
                    if (widget != null)
                    {
                        ...
                        {
                            btn.setHandler(b -> PacketDistributor.sendToServer(new WidgetTriggerMessage(widget)));
    ...
    /**
     * Rebuild the favorite list in place after a slot change.
     */
